...
export function EditDraftModal({ isOpen, onClose, problem, onSave, onDelete }: EditDraftModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-0 md:p-4">
      ...
      <div className="relative bg-white shadow-2xl w-full h-full md:rounded-xl md:max-w-3xl md:max-h-[85vh] md:h-auto flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-900">Edit Draft</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {activeTab === 'details' && (
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="draft-description" className="block text-sm font-medium text-gray-700 mb-1">
          ...
          {activeTab === 'yaml' && (
            <div className="space-y-3">
              <textarea
                ...
                className="w-full h-96 px-3 py-2 font-mono text-xs border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                spellCheck={false}
              />
              {yamlError && (
                <div className="p-3 bg-red-50 border border-red-200 rounded-lg">
                  <p className="text-sm text-red-800 whitespace-pre-wrap">{yamlError}</p>
                </div>
              )}
            </div>
          )}
        ...
        {showDeleteConfirm && (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-xl">
            ...
            <div className="relative bg-white rounded-lg shadow-xl p-6 max-w-sm w-full mx-6">
  ...
  return (
    <div className={`border-l-3 rounded-lg border ${stageColor} overflow-hidden`}>
      ...
      >
        ...
        <span className="text-xs text-gray-400 shrink-0">{node.id}</span>
        <svg
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-gray-100">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1">
              Content
              <span className="font-normal text-gray-400 ml-1">({node.content.length} chars)</span>
            </label>
            <textarea
              ref={contentRef}
              value={node.content}
              ...
              className="w-full px-2.5 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none min-h-[60px]"
              rows={3}
            />
          </div>
                  ...
                  <div key={idx} className="bg-gray-50 rounded p-2 space-y-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 px-2 py-1 border border-gray-300 rounded text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="Choice label"
                      />
                      <span className="text-xs text-gray-400 shrink-0" title={`Next: ${choice.next}`}>
                        → {choice.next}
                      </span>
                    </div>
                    <ChoiceAnswerEditor
          ...
          {/* Terminal badge */}
          {node.type === 'terminal' && (
            <div className="text-xs text-gray-400 italic">Terminal node (end of path)</div>
          )}
        </div>
      )}
    </div>
...
function ChoiceAnswerEditor({ answer, onChange }: { answer: string; onChange: (v: string) => void }) {
  ...
  return (
    <textarea
      ...
      className="w-full ml-6 px-2 py-1 border border-gray-200 rounded text-xs text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none min-h-[32px]"
      rows={1}

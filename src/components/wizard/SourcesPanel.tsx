    ...
    <div className="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">Sources ({sources.length})</h3>
        </div>
        <div className="flex items-center gap-2">
          {sources.length > 0 && (
            <button
      ...
      <div className="p-3 sm:p-4 space-y-3 max-h-96 overflow-y-auto">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            ...
            className="text-xs px-3 py-1.5 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg font-medium transition-colors disabled:opacity-50 flex items-center gap-1"
          >
            {searchLoading ? (
        ...
        {(showJinaKey || (!jinaKey && error?.includes('API key'))) && (
          <div className="flex gap-2 items-center">
            <input
              ...
              className="flex-1 text-xs border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent font-mono bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            />
            <button
              onClick={() => {
        ...
        {addMode === 'url' && (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              autoFocus
            />
            <button
        ...
        {addMode === 'text' && (
          <div className="space-y-2">
            <input
              ...
              className="w-full text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
              autoFocus
            />
            <textarea
              value={textContent}
              ...
              className="w-full h-24 text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
            />
            <button
        ...
        {/* Error */}
        {error && (
          <div className="text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2">
            {error}
          </div>
        )}
        ...
        {sources.length > 0 && (
          <div className="space-y-2">
            {sources.map((source, idx) => (
              <div key={source.id} className="flex items-start gap-2 p-2 bg-gray-50 dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700">
                <span className="text-xs font-bold text-gray-500 dark:text-gray-400 mt-0.5 flex-shrink-0 w-5 text-center">
                  [{idx + 1}]
                </span>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {source.summary && (
                      <span className="text-xs text-green-600 dark:text-green-400 font-medium">Summarized</span>
                    )}
                  </div>
                </div>
                <button
                  onClick={() => onRemoveSource(source.id)}
                  className="p-1 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:hover:text-red-400 rounded transition-colors flex-shrink-0"
                  title="Remove source"
                >
        ...
        {searchResults.length > 0 && (
          <div className="space-y-2">
            ...
            {searchResults.slice(0, 5).map((result, idx) => (
              <div key={idx} className="flex items-start gap-2 p-2 bg-purple-50 dark:bg-purple-900/30 rounded-lg border border-purple-200 dark:border-purple-800">
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-gray-800 dark:text-gray-200 truncate">{result.title}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 line-clamp-2 mt-0.5">{result.snippet}</div>
                  {result.url && (
                    <div className="text-xs text-gray-400 dark:text-gray-500 truncate mt-0.5">{truncateUrl(result.url)}</div>
                  )}
                </div>
                <button
